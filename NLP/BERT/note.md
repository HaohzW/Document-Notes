...
语言模型预训练能提升自然语言处理任务的效果（例如词嵌入，GPT等）
...
​	BERT采用**基于掩码的语言模型（masked language model，MLM）**【MLM随机遮挡输入的某些token，目标是基于上下文预测该位置的原始token】，这样便结合了双向的信息；此外，BERT还做了“预测下一个句子”的任务，能让模型学习句子层面的信息；
